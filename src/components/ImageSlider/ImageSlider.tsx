...
const ImageSlider: FC<ImageSliderType> = ({ match }) => {
  //state
  ...
  const onClickBack = () => {
    ...
    if (match?.params?.userID === currentUser?.uid) {
      history.push(
        `/users/${match?.params?.userID}/collection_private/${match?.params?.collection_private_id}`
      );
    } else {
      history.push("/");
    }
  ...
  useEffect(() => {
    if (onClickIsTrue) {
      history.push(
        `/users/${match?.params?.userID}/collection_private/${match?.params?.collection_private_id}/image/${images[imageIndex]?.id}/${imageIndex}`
      );
